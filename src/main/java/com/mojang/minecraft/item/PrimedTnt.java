...
public class PrimedTnt extends Entity {
    ...
    public PrimedTnt(Level level, float x, float y, float z) {
        ...
        this.setPos(x, y, z);
        float level1 = (float)(Math.random() * Math.PI * 2.0D);
        this.xd = -((float)Math.sin((double)level1 * Math.PI / 180.0D)) * 0.02F;
        this.yd = 0.2F;
        this.zd = -((float)Math.cos((double)level1 * Math.PI / 180.0D)) * 0.02F;
