...
public class Item extends Entity {
	...
	public void render(Textures texture, float translation) {
		...
		int texture1 = texture.loadTexture("/terrain.png");
		GL11.glBindTexture(GL11.GL_TEXTURE_2D, texture1);
		float texture2 = this.level.getBrightness((int)this.x, (int)this.y, (int)this.z);
		float f3 = this.rot + ((float)this.tickCount + translation) * 3.0F;
		GL11.glPushMatrix();
		GL11.glColor4f(texture2, texture2, texture2, 1.0F);
		texture2 = (float)Math.sin((double)(f3 / 10.0F)) * 0.1F + 0.1F;
		GL11.glTranslatef(this.xo + (this.x - this.xo) * translation, this.yo + (this.y - this.yo) * translation + texture2, this.zo + (this.z - this.zo) * translation);
		GL11.glRotatef(f3, 0.0F, 1.0F, 0.0F);
		models[this.resource].render();
		GL11.glDisable(GL11.GL_TEXTURE_2D);
		texture2 = (texture2 = (texture2 = (float)Math.sin((double)(f3 / 10.0F)) * 0.5F + 0.5F) * texture2) * texture2;
		GL11.glColor4f(1.0F, 1.0F, 1.0F, texture2 * 0.4F);
	...
	public void playerTouch(Player player1) {
		if(player1.addResource(this.resource)) {
			this.level.addEntity(new TakeItemAnim(this.level, this, player1));
			this.remove();
		}
