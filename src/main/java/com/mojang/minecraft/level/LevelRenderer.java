...
public class LevelRenderer implements LevelListener {
    ...
    public void renderHit(HitResult h, int mode, int tileType) {
        ...
        if(mode == 0) {
            ...
            float var10 = (float)Math.sin((double)EagRuntime.currentTimeMillis() / 100.0D) * 0.2F + 0.8F;
            GL11.glColor4f(var10, var10, var10, (float)Math.sin((double)EagRuntime.currentTimeMillis() / 200.0D) * 0.2F + 0.5F);
            GL11.glEnable(GL11.GL_TEXTURE_2D);
            int id = this.textures.loadTexture("/terrain.png", 9728);
            GL11.glBindTexture(GL11.GL_TEXTURE_2D, id);
            int x = h.x;
            ...
            t.init(DefaultVertexFormats.POSITION_TEX);
            t.noColor();
