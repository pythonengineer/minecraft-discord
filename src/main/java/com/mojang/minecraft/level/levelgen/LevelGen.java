...
public final class LevelGen {
	...
	public final Level generateLevel(String creator, int width, int height, int depth) {
		...
		for(i14 = 0; i14 < i46; ++i14) {
			...
			float f48 = levelGen6.random.nextFloat() * (float)i43;
			float f49 = levelGen6.random.nextFloat() * (float)i32;
			float f50 = levelGen6.random.nextFloat() * (float)i44;
			i52 = (int)((levelGen6.random.nextFloat() + levelGen6.random.nextFloat()) * 200.0F);
			float f53 = (float)((double)levelGen6.random.nextFloat() * 3.141592653589793D * 2.0D);
			float f54 = 0.0F;
			float f21 = (float)((double)levelGen6.random.nextFloat() * 3.141592653589793D * 2.0D);
			float f55 = 0.0F;
			...
			for(int i7 = 0; i7 < i52; ++i7) {
				...
				f53 += f54 * 0.2F;
				f54 = (f54 *= 0.9F) + (levelGen6.random.nextFloat() - levelGen6.random.nextFloat());
				f21 = (f21 + f55 * 0.5F) * 0.5F;
				f55 = (f55 *= 0.75F) + (levelGen6.random.nextFloat() - levelGen6.random.nextFloat());
				if(levelGen6.random.nextFloat() >= 0.25F) {
					float f37 = f48 + (levelGen6.random.nextFloat() * 4.0F - 2.0F) * 0.2F;
					float f42 = f49 + (levelGen6.random.nextFloat() * 4.0F - 2.0F) * 0.2F;
					float f12 = f50 + (levelGen6.random.nextFloat() * 4.0F - 2.0F) * 0.2F;
					float f24 = ((float)levelGen6.depth - f42) / (float)levelGen6.depth;
					f24 = 1.2F + (f24 * 3.5F + 1.0F) * f23;
					f24 = (float)(Math.sin((double)i7 * 3.141592653589793D / (double)i52) * (double)f24);
						...
						for(int i26 = (int)(f42 - f24); i26 <= (int)(f42 + f24); ++i26) {
							for(int i27 = (int)(f12 - f24); i27 <= (int)(f12 + f24); ++i27) {
								...
								if(f28 * f28 + f29 * f29 * 2.0F + f30 * f30 < f24 * f24 && i25 >= 1 && i26 >= 1 && i27 >= 1 && i25 < levelGen6.width - 1 && i26 < levelGen6.depth - 1 && i27 < levelGen6.height - 1) {
									...
									if(levelGen6.blocks[i56] == Tile.rock.id) {
		...
		for(i15 = 0; i15 < i14; ++i15) {
			...
			i47 = levelGen6.random.nextInt(levelGen6.width);
			i17 = levelGen6.waterLevel - 1 - levelGen6.random.nextInt(2);
			i52 = levelGen6.random.nextInt(levelGen6.height);
		...
		Level level34;
		(level34 = new Level()).waterLevel = this.waterLevel;
		level34.setData(width, 64, height, this.blocks);
		level34.createTime = EagRuntime.currentTimeMillis();
		level34.creator = creator;
		...
		i43 = level34.maybeSpawnMobs(i38, (Entity)null, this.levelLoaderListener);
	...
	private void addOre(int tile, int rarity, int min, int max) {
		...
		for(int i8 = 0; i8 < i7; ++i8) {
			...
			float f13 = (float)((double)this.random.nextFloat() * 3.141592653589793D * 2.0D);
			float f14 = 0.0F;
			float f15 = (float)((double)this.random.nextFloat() * 3.141592653589793D * 2.0D);
			float f16 = 0.0F;
			...
			for(int i17 = 0; i17 < i12; ++i17) {
				...
				float f18 = (float)(Math.sin((double)i17 * 3.141592653589793D / (double)i12) * (double)rarity / 100.0D + 1.0D);
	...
	private void addLava() {
		...
		for(int i3 = 0; i3 < i2; ++i3) {
			...
			if(this.blocks[(i5 * this.height + i6) * this.width + i4] == 0) {
				++i1;
				this.floodFillWithLiquid(i4, i5, i6, 0, Tile.calmLava.id);
			}
		}
