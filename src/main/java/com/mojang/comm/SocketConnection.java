            ...
            for(int i3 = 0; i3 < object2.length; ++i3) {
                ...
                if(this.connected) {
                    try {
                        if(class5 == Long.TYPE) {
                            ...
                            if(class5 != String.class) {
                                if(class5 == byte[].class) {
                                    if((b9 = (byte[])((byte[])object6)).length < 1024) {
                                        b9 = Arrays.copyOf(b9, 1024);
                                    }

                                    socketConnection4.writeBuffer.put(b9);
                                }
                            } else {
                                b9 = ((String)object6).getBytes(Charset.forName("UTF-8"));
                                Arrays.fill(socketConnection4.stringPacket, (byte)32);

                                int i10;
                                for(i10 = 0; i10 < 64 && i10 < b9.length; ++i10) {
                                    socketConnection4.stringPacket[i10] = b9[i10];
                                }

                                for(i10 = b9.length; i10 < 64; ++i10) {
                                    socketConnection4.stringPacket[i10] = 32;
                                }
                        ...
                        ConnectionManager connectionManager8;
                        (connectionManager8 = this.manager).connection.disconnect();
                    }
                }
            }
        ...
        if(!this.connected) {
            ...
            try {
                if(class1 == Long.TYPE) {
                    ...
                    return (new String(this.stringPacket, Charset.forName("UTF-8"))).trim();
                } else if(class1 == byte[].class) {
                    byte[] b4 = new byte[1024];
                    this.readBuffer.get(b4);
                    return b4;
                } else {
                    return null;
                }
                ...
                (connectionManager3 = this.manager).connection.disconnect();
                return null;
            }
        }
    }
