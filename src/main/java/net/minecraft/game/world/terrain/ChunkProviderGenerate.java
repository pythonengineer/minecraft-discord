...
public final class ChunkProviderGenerate implements IChunkProvider {
	...
	public final Chunk provideChunk(int chunkX, int chunkZ) {
		...
		ChunkProviderGenerate chunkProviderGenerate71 = this;
		if(d6 == null) {
			d6 = new double[425];
		}
		...
		for(i8 = 0; i8 < 5; ++i8) {
			for(int i9 = 0; i9 < 5; ++i9) {
				for(int i10 = 0; i10 < 17; ++i10) {
					double d63;
					if((d63 = ((double)i10 - 8.5D) * 12.0D) < 0.0D) {
						d63 *= 2.0D;
					}

					double d65 = chunkProviderGenerate71.noise1[i7] / 512.0D;
					double d67 = chunkProviderGenerate71.noise2[i7] / 512.0D;
					double d61;
					double d69;
					if((d69 = (chunkProviderGenerate71.noise3[i7] / 10.0D + 1.0D) / 2.0D) < 0.0D) {
						d61 = d65;
					} else if(d69 > 1.0D) {
						d61 = d67;
					} else {
		...
		for(i72 = 0; i72 < 4; ++i72) {
			for(i73 = 0; i73 < 4; ++i73) {
				for(i7 = 0; i7 < 16; ++i7) {
					double d75 = this.noiseArray[(i72 * 5 + i73) * 17 + i7];
					double d77 = this.noiseArray[(i72 * 5 + i73 + 1) * 17 + i7];
					double d12 = this.noiseArray[((i72 + 1) * 5 + i73) * 17 + i7];
					double d14 = this.noiseArray[((i72 + 1) * 5 + i73 + 1) * 17 + i7];
					double d16 = this.noiseArray[(i72 * 5 + i73) * 17 + i7 + 1];
					double d18 = this.noiseArray[(i72 * 5 + i73 + 1) * 17 + i7 + 1];
					double d20 = this.noiseArray[((i72 + 1) * 5 + i73) * 17 + i7 + 1];
					double d22 = this.noiseArray[((i72 + 1) * 5 + i73 + 1) * 17 + i7 + 1];

					for(int i24 = 0; i24 < 8; ++i24) {
						double d25 = (double)i24 / 8.0D;
						double d27 = d75 + (d16 - d75) * d25;
						double d29 = d77 + (d18 - d77) * d25;
						double d31 = d12 + (d20 - d12) * d25;
						double d33 = d14 + (d22 - d14) * d25;

						for(int i82 = 0; i82 < 4; ++i82) {
							double d36 = (double)i82 / 4.0D;
							double d38 = d27 + (d31 - d27) * d36;
							double d40 = d29 + (d33 - d29) * d36;
							int i26 = i82 + (i72 << 2) << 11 | 0 + (i73 << 2) << 7 | (i7 << 3) + i24;
							...
							for(int i35 = 0; i35 < 4; ++i35) {
								...
								int i83 = 0;
								if((i7 << 3) + i24 < 64) {
									i83 = Block.waterStill.blockID;
								}

								if(d46 > 0.0D) {
									i83 = Block.stone.blockID;
								}
		...
		for(i72 = 0; i72 < 16; ++i72) {
			for(i73 = 0; i73 < 16; ++i73) {
				double d74 = (double)((chunkX << 4) + i72);
				double d76 = (double)((chunkZ << 4) + i73);
				boolean z13 = this.noiseGen4.generateNoiseOctaves(d74 * 8.0D / 256D, d76 * 8.0D / 256D, 0.0D) + this.rand.nextDouble() * 0.2D > 0.0D;
				boolean z78 = this.noiseGen4.generateNoiseOctaves(d76 * 8.0D / 256D, 109.0134D, d74 * 8.0D / 256D) + this.rand.nextDouble() * 0.2D > 3.0D;
				int i15 = (int)(this.noiseGen5.generateNoiseOctaves(d74 * 8.0D / 256D * 2.0D, d76 * 8.0D / 256D * 2.0D) / 3.0D + 3.0D + this.rand.nextDouble() * 0.25D);
				int i79 = i72 << 11 | i73 << 7 | 127;
				int i17 = -1;
				int i80 = Block.grass.blockID;
				int i19 = Block.dirt.blockID;

				for(int i81 = 127; i81 >= 0; --i81) {
					if(b3[i79] == 0) {
						i17 = -1;
					} else if(b3[i79] == Block.stone.blockID) {
						if(i17 == -1) {
							if(i15 <= 0) {
								i80 = 0;
								i19 = (byte)Block.stone.blockID;
							} else if(i81 >= 60 && i81 <= 65) {
								i80 = Block.grass.blockID;
	...
	public final void populate(IChunkProvider chunkProvider, int chunkX, int chunkZ) {
		...
		if((chunkZ = (int)(this.mobSpawnerNoise.generateNoiseOctaves((double)i8 * 0.05D, (double)chunkX * 0.05D) - this.rand.nextDouble())) < 0) {
			chunkZ = 0;
		}
