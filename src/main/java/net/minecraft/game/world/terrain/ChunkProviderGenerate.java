...
public final class ChunkProviderGenerate implements IChunkProvider {
	...
	public final Chunk provideChunk(int chunkX, int chunkZ) {
		...
		for(i83 = 0; i83 < 4; ++i83) {
			for(i84 = 0; i84 < 4; ++i84) {
				for(i7 = 0; i7 < 16; ++i7) {
					double d86 = this.noiseArray[(i83 * 5 + i84) * 17 + i7];
					double d10 = this.noiseArray[(i83 * 5 + i84 + 1) * 17 + i7];
					double d12 = this.noiseArray[((i83 + 1) * 5 + i84) * 17 + i7];
					double d14 = this.noiseArray[((i83 + 1) * 5 + i84 + 1) * 17 + i7];
					double d16 = this.noiseArray[(i83 * 5 + i84) * 17 + i7 + 1];
					double d18 = this.noiseArray[(i83 * 5 + i84 + 1) * 17 + i7 + 1];
					double d20 = this.noiseArray[((i83 + 1) * 5 + i84) * 17 + i7 + 1];
					double d22 = this.noiseArray[((i83 + 1) * 5 + i84 + 1) * 17 + i7 + 1];

					for(int i24 = 0; i24 < 8; ++i24) {
						double d25 = (double)i24 / 8.0D;
						double d27 = d86 + (d16 - d86) * d25;
						double d29 = d10 + (d18 - d10) * d25;
						double d31 = d12 + (d20 - d12) * d25;
						double d33 = d14 + (d22 - d14) * d25;

						for(int i93 = 0; i93 < 4; ++i93) {
							double d36 = (double)i93 / 4.0D;
							double d38 = d27 + (d31 - d27) * d36;
							double d40 = d29 + (d33 - d29) * d36;
							int i26 = i93 + (i83 << 2) << 11 | 0 + (i84 << 2) << 7 | (i7 << 3) + i24;
							...
							for(int i35 = 0; i35 < 4; ++i35) {
								...
								int i94 = 0;
								if((i7 << 3) + i24 < 64) {
									i94 = Block.waterStill.blockID;
								}

								if(d46 > 0.0D) {
									i94 = Block.stone.blockID;
								}
		...
		for(i83 = 0; i83 < 16; ++i83) {
			for(i84 = 0; i84 < 16; ++i84) {
				double d85 = (double)((chunkX << 4) + i83);
				double d88 = (double)((chunkZ << 4) + i84);
				boolean z13 = this.noiseGen4.generateNoiseOctaves(d85 * 8.0D / 256D, d88 * 8.0D / 256D, 0.0D) + this.rand.nextDouble() * 0.2D > 0.0D;
				boolean z89 = this.noiseGen4.generateNoiseOctaves(d88 * 8.0D / 256D, 109.0134D, d85 * 8.0D / 256D) + this.rand.nextDouble() * 0.2D > 3.0D;
				int i15 = (int)(this.noiseGen5.generateNoiseOctaves(d85 * 8.0D / 256D * 2.0D, d88 * 8.0D / 256D * 2.0D) / 3.0D + 3.0D + this.rand.nextDouble() * 0.25D);
				int i90 = i83 << 11 | i84 << 7 | 127;
				int i17 = -1;
				int i91 = Block.grass.blockID;
				int i19 = Block.dirt.blockID;

				for(int i92 = 127; i92 >= 0; --i92) {
					if(i92 <= this.rand.nextInt(5)) {
						b3[i90] = (byte)Block.bedrock.blockID;
					} else if(b3[i90] == 0) {
						i17 = -1;
					} else if(b3[i90] == Block.stone.blockID) {
						if(i17 == -1) {
							if(i15 <= 0) {
								i91 = 0;
								i19 = (byte)Block.stone.blockID;
							} else if(i92 >= 60 && i92 <= 65) {
								i91 = Block.grass.blockID;
		...
		long j95 = (this.rand.nextLong() / 2L << 1) + 1L;
		long j96 = (this.rand.nextLong() / 2L << 1) + 1L;

		for(chunkX -= 8; chunkX <= i84 + 8; ++chunkX) {
			for(chunkZ = i7 - 8; chunkZ <= i7 + 8; ++chunkZ) {
				chunkProviderGenerate5.rand.setSeed((long)chunkX * j95 + (long)chunkZ * j96 ^ chunkProviderGenerate5.worldObj.seed);
				int i82 = chunkProviderGenerate5.rand.nextInt(10);
				if(chunkProviderGenerate5.rand.nextInt(3) != 0) {
					i82 = 0;
				}

				for(i9 = 0; i9 < i82; ++i9) {
					double d97 = (double)((chunkX << 4) + chunkProviderGenerate5.rand.nextInt(16));
					double d98 = (double)chunkProviderGenerate5.rand.nextInt(chunkProviderGenerate5.rand.nextInt(120) + 8);
					double d66 = (double)((chunkZ << 4) + chunkProviderGenerate5.rand.nextInt(16));
					if(chunkProviderGenerate5.rand.nextInt(4) == 0) {
						chunkProviderGenerate5.generateCaves(i84, i7, b87, d97, d98, d66, 1.0F + chunkProviderGenerate5.rand.nextFloat() * 6.0F, 0.0F, 0.0F, -1, -1, 0.5D);
					}

					float f68 = chunkProviderGenerate5.rand.nextFloat() * (float)Math.PI * 2.0F;
					float f99 = (chunkProviderGenerate5.rand.nextFloat() - 0.5F) * 2.0F / 8.0F;
					float f70 = chunkProviderGenerate5.rand.nextFloat() * 2.0F;
					chunkProviderGenerate5.generateCaves(i84, i7, b87, d97, d98, d66, f70, f68, f99, 0, 0, 1.0D);
				}
			}
		}
	...
	private void generateCaves(int chunkX, int chunkZ, byte[] chunkData, double x, double y, double z, float scaleFactor, float directionHorizontal, float directionVertical, int outwardsSize, int inwardsSize, double radius) {
		...
		while(true) {
			...
			for(boolean z26 = random23.nextInt(6) == 0; outwardsSize < inwardsSize; ++outwardsSize) {
				...
				f21 += (random23.nextFloat() - random23.nextFloat()) * random23.nextFloat() * 3.0F;
					...
					if(x >= d17 - 16.0D - d27 * 2.0D && z >= d19 - 16.0D - d27 * 2.0D && x <= d17 + 16.0D + d27 * 2.0D && z <= d19 + 16.0D + d27 * 2.0D) {
						...
						for(i40 = i53; !z57 && i40 < i34; ++i40) {
							for(int i41 = i56; !z57 && i41 < i38; ++i41) {
								int i42 = ((i40 << 4) + i41 << 7) + i36;

								for(i51 = i36 - 1; i51 >= i55; --i51) {
									if(chunkData[i42] == Block.waterMoving.blockID || chunkData[i42] == Block.waterStill.blockID) {
										z57 = true;
									}

									if(i51 != i55 && i40 != i53 && i40 != i34 - 1 && i41 != i56 && i41 != i38 - 1) {
										i51 = i55 + 1;
									}
								}
							}
